    ...
    <div class="page-container">
      <div class="page-header">
        <div class="header-left">
          ...
          <div>
            ...
            <p>Années utilisées pour organiser les emplois du temps ({{ annees.length }} enregistrées)</p>
          </div>
        </div>
        <button class="btn btn-primary" (click)="showAddForm()"><i class="pi pi-plus"></i> Nouvelle Année</button>
      </div>

      <div class="form-card" *ngIf="displayForm">
        <div class="form-card-header"><h3>{{ editingId ? 'Modifier l’Année' : 'Nouvelle Année' }}</h3></div>
        <div class="form-card-body">
          <div *ngIf="errorMessage" class="error-banner"><i class="pi pi-exclamation-triangle"></i>{{ errorMessage }}</div>
          <div class="section-grid">
            <div class="input-group"><label>Libellé</label><input type="text" [(ngModel)]="currentAnnee.libelle" placeholder="Ex: 2025-2026" /></div>
            <div class="input-group"><label>Date début</label><input type="date" [(ngModel)]="currentAnnee.dateDebut" /></div>
            <div class="input-group"><label>Date fin</label><input type="date" [(ngModel)]="currentAnnee.dateFin" /></div>
            <div class="input-group">
              <label>Statut</label>
              <select [(ngModel)]="currentAnnee.active">
      ...
      <div class="search-section">
        <div class="search-wrapper">
          ...
          <input type="text" placeholder="Rechercher une année..." [(ngModel)]="searchText" (input)="filterAnnees()" />
        </div>
      </div>
      ...
      <div class="cards-grid" *ngIf="filteredAnnees.length > 0">
        <div class="ref-card" *ngFor="let annee of filteredAnnees">
          ...
          <div class="card-body">
            <span [class]="annee.active ? 'badge-active' : 'badge-inactive'">{{ annee.active ? 'Active' : 'Inactive' }}</span>
            <div class="card-title">{{ annee.libelle }}</div>
            <div class="detail-item"><i class="pi pi-calendar-plus"></i><span>Début : {{ annee.dateDebut | date:'dd/MM/yyyy' }}</span></div>
            <div class="detail-item"><i class="pi pi-calendar-minus"></i><span>Fin : {{ annee.dateFin | date:'dd/MM/yyyy' }}</span></div>
          </div>
          <div class="card-actions">
            <button class="btn-icon-sm edit" (click)="showEditForm(annee)"><i class="pi pi-pencil"></i></button>
            <button class="btn-icon-sm delete" (click)="delete(annee.id!)"><i class="pi pi-trash"></i></button>
          </div>
        </div>
      </div>
...
export class AnneesUniversitairesComponent implements OnInit {
  ...
  showAddForm(): void {
    this.editingId = null;
    this.errorMessage = '';
    this.currentAnnee = { libelle: '', dateDebut: '', dateFin: '', active: false };
  ...
  save(): void {
    const anneeToSave: AnneeUniversitaire = {
      ...
      active: Boolean(this.currentAnnee.active),
    };
  ...
  delete(id: number): void {
    ...
    this.confirmDeleteMessage = 'Voulez-vous vraiment supprimer cette année ? Les emplois du temps liés peuvent empêcher la suppression.';
  }
