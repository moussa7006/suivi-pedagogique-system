...
export class ApiErrorService {
  ...
  private messageForStatus(status: number, fallback: string): string {
    if (status === 0) {
      const apiUrl = this.apiConfig.getBaseUrl() || 'non configurée';
      return `Impossible de joindre le serveur (${apiUrl}). Vérifiez l'IP, le Wi-Fi et que le backend écoute sur le port 8099.`;
    }
